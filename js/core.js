...
const matchesData = [
    // الجولات الثلاث الأولى (73 مباراة)
...
async function init() {
    ...
    if (typeof window.renderLeaderboard === 'function') window.renderLeaderboard('all');
    if (typeof window.renderPreviousGamesFiltered === 'function') window.renderPreviousGamesFiltered();
    if (typeof window.calculateStandings === 'function') window.calculateStandings();
    if (typeof window.renderScorers === 'function') window.renderScorers();
    if (typeof window.renderTeamStats === 'function') window.renderTeamStats();
    if (typeof window.renderAllPredictions === 'function') window.renderAllPredictions();
    if (typeof window.renderBracket === 'function') window.renderBracket();
    if (typeof window.updateNewsTicker === 'function') window.updateNewsTicker();
    if (typeof window.initTabs === 'function') window.initTabs();
    if (typeof window.checkUrlForMatch === 'function') window.checkUrlForMatch();
...
window.state = state;
window.userPredictionsMap = userPredictionsMap;
window.isAuthorized = isAuthorized;
window.SECRET_CODE = SECRET_CODE;
window.now = now;
window.matchTime = matchTime;
window.MATCH_DURATION = MATCH_DURATION;
window.isMatchLive = isMatchLive;
