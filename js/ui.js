// ============================================================
// ui.js - واجهة المستخدم (كامل ومصحح)
// ============================================================

// ===== عرض المباريات القادمة =====
function renderUpcoming() {
    try {
        console.log("📊 جاري عرض المباريات القادمة...");
        ...
        console.log("✅ تم عرض " + active.length + " مباراة");
    } catch (e) {
        console.error("❌ renderUpcoming:", e);
        document.getElementById('matchesContainer').innerHTML = `<div class="empty-state"><span class="icon">⚠️</span> حدث خطأ: ${e.message}</div>`;
    }
}
        ...
        <div class="match-card ${matchClass}" onclick="${isFinished ? `window.openMatchPredictions('${matchId}', '${m.team1}', '${m.team2}', ${homeScore}, ${awayScore})` : ''}">
            ...
            ${isUpcoming ? `
                <div class="predict-btn-wrap">
                    ...
                    <button class="view-btn" onclick="event.stopPropagation();window.openViewPredictionsModal('${matchId}','${m.team1}','${m.team2}')">
                        📋 استعراض التوقعات
                    </button>
                    <button class="share-link-btn" onclick="event.stopPropagation();window.copyMatchLink('${m.id}', '${m.team1}', '${m.team2}')">
                        🔗 مشاركة
                    </button>
                </div>
            ` : ''}
...
async function openMatchPredictions(matchId, team1, team2, homeScore, awayScore) {
    ...
    predictions.forEach((p, idx) => {
        ...
        rows += `<tr>
            <td class="user-name" onclick="window.openPlayerPredictions('${p.user_name || ''}')">${p.user_name || 'مجهول'}</td>
...
async function openViewPredictionsModal(matchId, team1, team2) {
    console.log("📋 استعراض توقعات المباراة:", matchId);
...
async function openPlayerPredictions(userName) {
    if (!userName) { 
        showCopyToast('⚠️ اسم المستخدم غير معروف'); 
        return; 
    }
    
    console.log("👤 فتح توقعات اللاعب:", userName);
...
function openTeamStatsModal(team1, team2) {
    console.log("📊 فتح إحصائيات الفريقين:", team1, team2);
...
function openCompareModal(selectedPlayer) {
    console.log("📊 فتح المقارنة:", selectedPlayer);
...
function openNameModal(matchId, team1, team2, timeISO) {
    if (isMatchFinished(timeISO)) { 
        showCopyToast('⛔ هذه المباراة انتهت، لا يمكن التوقع.'); 
        return; 
    }
    if (!canPredict(timeISO)) { 
        showCopyToast('⛔ لا يمكن التوقع الآن، المباراة على وشك البدء.'); 
        return; 
    }
...
document.getElementById('nameSubmitBtn').addEventListener('click', async function() {
    ...
    if (!name) { 
        errorEl.textContent = '⚠️ الرجاء إدخال اسمك'; 
        return; 
    }
...
document.getElementById('modalSubmitBtn').addEventListener('click', async function() {
    ...
    if (isMatchFinished(currentTimeISO)) { 
        msgEl.textContent = '⛔ هذه المباراة انتهت، لا يمكن حفظ التوقع.'; 
        msgEl.className = 'modal-message error'; 
        return; 
    }
    if (!canPredict(currentTimeISO)) { 
        msgEl.textContent = '⛔ لا يمكن التوقع الآن، المباراة على وشك البدء.'; 
        msgEl.className = 'modal-message error'; 
        return; 
    }
...
function toggleArchive() {
    ...
    archiveData.forEach(m => {
        ...
        if (m.hadPenalties && m.homePenalty !== null && m.awayPenalty !== null) {
            penaltyHtml = `<span class="score-penalty-badge">⚽ ركلات ترجيح: ${m.homePenalty} — ${m.awayPenalty}</span>`;
        }
        html += `
            <div class="archive-item" onclick="window.openPreviousMatchPredictions('${m.homeAr}', '${m.awayAr}', ${m.homeScore}, ${m.awayScore})">
                <div class="match-info">
...
function runTests() {
    ...
    setTimeout(() => {
        ...
        try {
            ...
            if (r1 && r2 && r3) { pass++; results.push('✅ canPredict - صحيح'); } 
            else { fail++; results.push('❌ canPredict - فشل'); }
        ...
        try {
            ...
            if (t1 && t2) { pass++; results.push('✅ translateToArabic - صحيح'); } 
            else { fail++; results.push('❌ translateToArabic - فشل'); }
...
window.updateShareAllCount = updateShareAllCount;
...
window.generateAnalyticsCharts = generateAnalyticsCharts;
